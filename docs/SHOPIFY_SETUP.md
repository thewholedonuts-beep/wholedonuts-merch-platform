...
5. Confirm only a `financial_status=paid` order creates a reward-ledger purchase entry, and that a refunded, voided, or cancelled order creates exactly one reversal.
6. Send fulfillment test events and confirm they update the existing Shopify order by `order_id` without creating a second order.
7. Replace the staging callback only after production secrets, DNS, TLS, and the database backup policy are in place.
